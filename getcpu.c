...
static const char *config_name = "thread_local_abi.cpu_id";
#elif CONFIG_THREAD_LOCAL_ABI_LAZY
static const char *config_name = "thread_local_abi.cpu_id (lazy)";
...
static inline int
thread_local_abi(uint32_t tlabi_nr, volatile struct thread_local_abi *tlabi,
...
static int init_thread_local_abi_cpu_id(void)
{
	int ret;

	ret = tlabi_cpu_id_register();
	if (ret < 0) {
		perror("getcpu_cache");
		return -1;
	}
...
{
	if (caa_unlikely(!(__thread_local_abi.features & TLABI_FEATURE_CPU_ID))) {
...
static void *thread_fct(void *arg)
{
	...
	sigsafe_fprintf(stderr, "[tid: %d, cpu: %d] Thread starts\n",
		gettid(), cpu);
	ret = init_thread_local_abi_cpu_id();
	if (ret) {
		abort();
	}
	...
	for (;;) {
#ifdef CONFIG_THREAD_LOCAL_ABI
		cputest = __thread_local_abi.cpu_id;
#elif CONFIG_THREAD_LOCAL_ABI_LAZY
